...
import (
	...
	"github.com/perlinleo/golang-todo/internal/middleware"
	"github.com/perlinleo/golang-todo/internal/model"
	responses "github.com/perlinleo/golang-todo/internal/pkg"
...
func NewUserHandler(router *router.Router, usecase user.Usecase) {
	...
	router.POST("/api/v1/user/{nickname}/create", middleware.ReponseMiddlwareAndLogger(handler.HandleCreateUser))
	router.GET("/api/v1/user/{nickname}/profile", middleware.ReponseMiddlwareAndLogger(handler.HandleGetUser))
	router.POST("/api/v1/user/{nickname}/profile", middleware.ReponseMiddlwareAndLogger(handler.HandleUpdateUser))
